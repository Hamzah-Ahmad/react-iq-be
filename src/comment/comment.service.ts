...
export class CommentService {
  ...
  async addCommentToSubmission(
    ...
    const foundSubmission = await this.submissionRepository.findOneBy({
      id: submissionId,
    });
    if (!foundSubmission)
      throw new NotFoundException('Submission with specified ID was not found');
  ...
  async deleteComment(commentId: string, userId: string) {
    ...
    if (comment.authorId !== userId && comment.submission.userId !== userId) {
      throw new ForbiddenException(
        null,
